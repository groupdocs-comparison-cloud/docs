...
# Get document comparison changes
...
curl.exe -v "https://api.groupdocs.cloud/v2.0/comparison/changes" `
  ...
  -d "{ 'SourceFile': { 'FilePath': 'source_files\\word\\source.docx' }, 'TargetFiles': [ { 'FilePath': 'target_files\\word\\target.docx' } ], 'Settings': { 'CalculateComponentCoordinates': true } }"
```
{{< /tab >}}
...
rem Get document comparison changes
curl -v "https://api.groupdocs.cloud/v2.0/comparison/changes" ^ 
  -X POST ^ 
  -H "Content-Type: application/json" ^ 
  -H "Accept: application/json" ^ 
